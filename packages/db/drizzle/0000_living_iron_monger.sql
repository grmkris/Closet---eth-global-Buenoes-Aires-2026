...
CREATE TABLE "wallet_address" (
	...
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "wallet_address_address_unique" UNIQUE("address")
);
--> statement-breakpoint
...
ALTER TABLE "ai_messages" ADD CONSTRAINT "ai_messages_user_id_user_id_fk" FOREIGN KEY ("user_id") REFERENCES "public"."user"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
CREATE INDEX "tag_usage_count_idx" ON "tag" USING btree ("usage_count");--> statement-breakpoint
